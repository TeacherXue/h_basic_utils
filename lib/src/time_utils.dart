...
class TimeUtils {
  ...
  ///结果:2019年07月21
  static String getOldDateStr(String startTime, int day, String format) {
    var date;
    //记录往后每一天的时间搓，用来和最后一天到做对比。这样就能知道什么时候停止了。
    DateTime startData = DateTime.parse(startTime);
    var mothFormatFlag =  DateFormat(format);
    var dateTime =  DateTime.fromMillisecondsSinceEpoch(
        startData.millisecondsSinceEpoch + day * 24 * 60 * 60 * 1000);
    String nowMoth = mothFormatFlag.format(dateTime);
    date.add(nowMoth);
    return date;
  }
  ...
  static List<String> getBeforeDayAndBeforeHour() {
    ...
    /// 更改当前时间的时间格式
    time.add(TimeUtils.getFormatDataString(now, 'yyyy-MM-dd'));

    /// 当前时间的前一天
    String _theDayBefore = TimeUtils.getFormatDataString(
        now.add( const Duration(days: -1)), 'yyyy-MM-dd');
    time.add(_theDayBefore);

    /// 当前时间的前一小时
    String _theHourBefore =
    TimeUtils.getFormatDataString(now.add( const Duration(hours: -1)), 'HH');
    time.add(_theHourBefore);

    /// 当前时间的小时
    String _hour = TimeUtils.getFormatDataString(now, "HH");
    time.add(_hour);
